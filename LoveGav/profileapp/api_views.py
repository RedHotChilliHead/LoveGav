...
from .models import User, Pet, Mood, Heat, Treatment
from .permissions import IsOwnerOrStaffReadOnly
from .serializers import UserSerializer, PetSerializer, MoodSerializer, HeatSerializer, TreatmentSerializer
...
class TreatmentViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        pet = Pet.objects.get(pk=self.kwargs['pk'])
        return Treatment.objects.filter(pet=pet)
